...
public class StateBarTranslucentUtils {
    ...
    public static void setStateBarColor(Activity activity,int color) {
        ...
        setupStatusBarView(activity, contentLayout, Color.parseColor("#FF68af65"));
    ...
    public static void setStateBarColor(Activity activity,String color) {
        // 设置状态栏颜色
        ViewGroup contentLayout = (ViewGroup) activity.findViewById(android.R.id.content);
        //setupStatusBarView(activity, contentLayout, color);
        setupStatusBarView(activity, contentLayout, Color.parseColor(color));
        // 设置Activity layout的fitsSystemWindows
        View contentChild = contentLayout.getChildAt(0);
        contentChild.setFitsSystemWindows(true);
    }
